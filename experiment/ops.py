...
def uk(input, k, reuse=False, norm='instance', is_training=True, name=None, image_width=None, image_height=None):
  ...
  Args:
    ...
    reuse: boolean
    name: string, e.g. 'c7sk-32'
    image_width: integer, desired output size of layer
  Returns:
    4D tensor
  """
  with tf.variable_scope(name, reuse=reuse):
    ...
    if not image_width:
      image_width = input_shape[1] * 2
    if not image_height:
      image_height = input_shape[2] * 2
    output_shape = [input_shape[0], image_width, image_height, k]
